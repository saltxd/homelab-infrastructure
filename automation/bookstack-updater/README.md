...
1. Log in to BookStack at http://docs.cluster.local
...
```bash
curl -H "Authorization: Token <id>:<secret>" http://docs.cluster.local/api/books
```
...
```bash
curl http://docs.cluster.local/login
```

## Related

- BookStack: http://docs.cluster.local
- Homarr Dashboard: http://dashboard.cluster.local
- Homer Dashboard: http://home.cluster.local
