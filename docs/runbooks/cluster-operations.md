...
ssh k3s-cp-1 "sudo etcdctl endpoint status --cluster -w table"
ssh k3s-cp-1 "sudo etcdctl endpoint health --cluster"
```
...
ssh k3s-cp-1 "sudo cat /var/lib/rancher/k3s/server/node-token"
...
```bash
# Create snapshot
ssh k3s-cp-1 "sudo k3s etcd-snapshot save --name manual-backup"

# List snapshots
ssh k3s-cp-1 "sudo k3s etcd-snapshot ls"
...
# Check member list
ssh k3s-cp-1 "sudo etcdctl member list -w table"
