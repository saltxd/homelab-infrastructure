...
### List All Books
```bash
curl -s http://docs.cluster.local/api/books \
...
```bash
curl -s http://docs.cluster.local/api/books/{BOOK_ID} \
  -H "Authorization: Token YOUR_TOKEN_ID:YOUR_TOKEN_SECRET" \
  | jq '.contents'
```
...
curl -X POST http://docs.cluster.local/api/pages \
  -H "Authorization: Token YOUR_TOKEN_ID:YOUR_TOKEN_SECRET" \
...
curl -X PUT http://docs.cluster.local/api/pages/{PAGE_ID} \
  -H "Authorization: Token YOUR_TOKEN_ID:YOUR_TOKEN_SECRET" \
  -H "Content-Type: application/json" \
  -d @/tmp/page.json
```
...
```bash
curl -X POST http://docs.cluster.local/api/chapters \
  -H "Authorization: Token YOUR_TOKEN_ID:YOUR_TOKEN_SECRET" \
  -H "Content-Type: application/json" \
...
curl -s http://docs.cluster.local/api/books/20 \
  -H "Authorization: Token YOUR_TOKEN_ID:YOUR_TOKEN_SECRET" \
